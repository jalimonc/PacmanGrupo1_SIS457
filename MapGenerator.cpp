...
MapGenerator::MapGenerator(int _anchoPantalla, int _altoPantalla)
{
	...
	paredvTexture = new Texture();
	paredvTexture->loadFromImage(pathParedv);
}
...
bool MapGenerator::load(string path)
{
	...
	while (getline(file, line)) {
		...
		for (unsigned int x = 0; x < chars.size(); x++) {
			...
			{
			case 'x':
				newObject = new pared(paredhTexture, x * 25, y * 25, 26, 40, anchoPantalla, altoPantalla);
				break;
			case 'y':
				newObject = new pared(paredvTexture, x * 25, y * 25, 25, 25, anchoPantalla, altoPantalla);
				break;
			case '.':
